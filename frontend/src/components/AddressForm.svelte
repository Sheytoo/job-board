    ...
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="py-9 px-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium text-gray-900">{creation ? "Create" : "Edit"} address</h3>
                <form class="space-y-6" on:submit|preventDefault={handleSubmit}>
                    ...
                    <div>
                        ...
                        <input type="text" name="street" id="street" value="{data.street}"
                    ...
                    <div>
                        ...
                        <input type="text" name="zip" id="zip" value="{data.zip}" maxlength="10"
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                               required/>
                    </div>
                    <div>
                        ...
                        <select name="country" id="country"
                            ...
                            {#each countries as country}
                                <option value="{country.name}"
                                        selected="{country.name === data.country}">{country.name}</option>
                            {/each}
                        </select>
                    </div>
                    <button type="submit"
