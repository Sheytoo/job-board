<script>
    ...
    function openDeleteModal(id) {
        dispatch("openDeleteModal", { name: "user", id: id });
    }

    function openEditModal(id) {
        dispatch("openEditModal", { name: "users", id: id });
    }
...
<table class="w-full text-sm text-left text-gray-500">
    ...
    <tr>
        ...
        <th scope="col" class="py-3 px-6">Is Admin</th>
        ...
        <tr class="bg-white border-b hover:bg-gray-50">
            ...
            <td class="flex py-4 px-6 text-right space-x-1.5">
                <button class="text-blue-500 transform hover:text-blue-600 hover:scale-110" title="Modify"
                        on:click|preventDefault={() => openEditModal(user.id)}>
                ...
                <button class="text-red-500 transform hover:text-red-600 hover:scale-110" title="Delete"
                        on:click|preventDefault={() => openDeleteModal(user.id)}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
